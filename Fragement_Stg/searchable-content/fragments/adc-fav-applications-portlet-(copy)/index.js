$(document).ready(function () {
  ...
  // Helper to recursively find a field by name
  function findFieldByName(fields, name) {
    for (let field of fields) {
      if (field.name === name) {
        return field;
      } else if (field.nestedContentFields && field.nestedContentFields.length > 0) {
        let nestedResult = findFieldByName(field.nestedContentFields, name);
  ...
  for (let article of articles.items) {
    // Get PreviewImage from nested content fields
    const previewImageField = findFieldByName(article.contentFields, "PreviewImage");
    const quickLinksLinkField = findFieldByName(article.contentFields, "quickLinksLink");

    const previewImageUrl = previewImageField?.contentFieldValue?.image?.contentUrl || null;
    const quickLinksLink = quickLinksLinkField?.contentFieldValue?.data || "#";

    objArr.push(`
      <div class="col-lg-4 col-md-6 v-space">
        <button articlePK="${article?.resourcePrimaryKey}" thumbsUp="${article?.ratings?.thumbsUp}" 
          ...
          <i class="${article.ratings?.thumbsUp ? 'icon-bookmark' : 'icon-bookmark-empty'}"></i>
        </button>
        <a class="btn btn-light applications-btn btn-block font-b" target="_blank" href="${quickLinksLink}">
          <img alt="" height="40" src="${previewImageUrl}"> ${article.title}
        </a>
      </div>
    `);
