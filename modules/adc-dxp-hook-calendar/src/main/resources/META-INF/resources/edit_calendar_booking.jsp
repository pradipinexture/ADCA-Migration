...
<aui:script>
    ...
    <c:if test="<%= invitable %>">
        ...
        var childCalendarIds = Object.keys(availableCalendars);
        ...
        // Remove the main calendar from child calendars to avoid duplication
        var mainCalendarIndex = childCalendarIds.indexOf(calendarId);
        if (mainCalendarIndex > -1) {
        childCalendarIds.splice(mainCalendarIndex, 1);
        }
        // Remove any duplicate calendar IDs
        childCalendarIds = childCalendarIds.filter((value, index, self) => {
        return self.indexOf(value) === index;
        });
        // Set the reinvitable calendar IDs
        var reinvitableCalendarIdsNode = document.getElementById(
        '<portlet:namespace />reinvitableCalendarIds'
        );
        if (reinvitableCalendarIdsNode) {
        // Remove duplicates from reinvitable list as well
        var uniqueReinvitableIds = window.<portlet:namespace />reinvitableCalendarIds.filter((value, index, self) => {
        return self.indexOf(value) === index;
        });
        reinvitableCalendarIdsNode.value = uniqueReinvitableIds.join(',');
        }
...
<aui:script use="json,liferay-calendar-date-picker-sanitizer,liferay-calendar-interval-selector,liferay-calendar-interval-selector-scheduler-event-link,liferay-calendar-list,liferay-calendar-recurrence-util,liferay-calendar-reminders,liferay-calendar-simple-menu,liferay-calendar-util">
    ...
    // Helper function to check for duplicates across all invitation lists
    var isCalendarAlreadyInvited = function(calendarId) {
    var allLists = [
    ...
    <c:if test="<%= calendarBooking != null %>">
        allLists.push(window.<portlet:namespace />calendarListMaybe);
    </c:if>

    for (var i = 0; i < allLists.length; i++) {
    var list = allLists[i];
    if (list && list.getCalendar && list.getCalendar(calendarId)) {
    return true;
    }
    }
    return false;
    ...
    // Add to reinvitable list only if not already there
    if (window.<portlet:namespace />reinvitableCalendarIds.indexOf(calendarId) === -1) {
    ...
    <c:if test="<%= invitable %>">
        ...
        // Updated addToList function with duplicate checking
        var addToList = function (calendar) {
        // Check if calendar is already invited
        if (isCalendarAlreadyInvited(calendar.calendarId)) {
        console.log('Calendar already invited:', calendar.name || calendar.calendarId);
        ...
        console.log('User/Calendar already invited');
        // You can uncomment the line below for user notification
        // alert('This user/calendar is already invited.');
        return;
        }
        ...
        if (calendar.classNameId == 10001) { // Organization handling
        var organizationCalendarsAdded = false;
        if (<%= cal_grp_ids != null %>) {
        <%
        ...
        %>
        var orgCalendar = <%= calendarJSONObject%>;
        // Check if this organization calendar is already invited
        if (!isCalendarAlreadyInvited(orgCalendar.calendarId)) {
        <portlet:namespace />calendarListPending.add(orgCalendar);
        organizationCalendarsAdded = true;
        }
        }
        <%
                    }
        ...
        // If no organization calendars were added, add the main calendar
        if (!organizationCalendarsAdded) {
        addToList(calendar);
        }
        } else {
        // For non-organization calendars, use the standard addToList function
        addToList(calendar);
        }
        }
        );
